    ...
    export default {
        ...
        methods: {
            handelChange(date) {
                ...
                this.title = `设计部${moment(date[0]).format('YYYY')}年${moment(date[0]).format('MM')}月${moment(date[0]).format('DD')}日-${moment(date[1]).format('DD')}日工作总结（${JSON.parse(localStorage.getItem('userInfo')).nickName}）`;
            },
            add(index) {
