...
<template>
    <Layout>
        <Sider :style="{ background: '#fff' }" width="220" ref="side">
            <Menu class="yggs-menu" :active-name="activeName" theme="light" ref="qd-menu" width="auto" :open-names="openName" @on-select="onSelect">
                <template v-for="item in menuList">
                    <template v-if="item.children.length===0">
                        <MenuItem :name="item.id" :key="item.id" replace :to="'/doc/qd?id='+item.id">{{item.name}}</MenuItem>
                    </template>
                    <template v-else>
                        <Submenu :name="item.id" :key="item.id">
                            <template slot="title">{{item.name}}</template>
                            <MenuItem :name="item2.id" :key="item2.id" replace :to="'/doc/qd?id='+item2.id" v-for="item2 in item.children">{{item2.name}}</MenuItem>
                        </Submenu>
                    </template>
                </template>
            </Menu>
...
export default {
    ...
    methods: {
        ...
        getActiveName() {
            ...
            this.$nextTick(()=> {
                this.$refs['qd-menu'].updateOpened();
                this.$refs['qd-menu'].updateActiveName();
            });
        },
        createMl() {
